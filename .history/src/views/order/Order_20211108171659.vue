	...
	<div id="order">
					...
					<template slot-scope="scope">
						...
						<el-tooltip effect="dark" content="删除" placement="top">
							...
							<el-button
								...
								circle
								size="mini"
								@click="delDialog(scope.row.order_num)"
							></el-button>
						</el-tooltip>
					</template>
				</el-table-column>
		...
		>
			...
			<span slot="footer" class="dialog-footer">
				<el-button type="primary" @click="_updateOrder">确 定</el-button>
...
export default {
	...
	methods: {
		...
		// 搜索订单号
		async _findOrder() {
			let res = await findOrder(this.queryInfo.query);
			if (res.success === true) {
				this.orderList = res.data;
			}
		},
		...
		// 点击按钮 修改订单
		async _updateOrder() {
			this.addForm.state = this.seletedStatusId;
			// this.$refs.addFormRef.validate(async (valid) => {
			// 	if (!valid) return;
			//表单都合法才向后台发起添加商品的请求
			// let params = {
			// 	order_num: this.addForm.order_num,
			// 	state: this.seletedStatusId,
			// 	total_price: this.addForm.total_price,
			// };
			let res = await updateOrder(this.addForm);
			console.log(res);
			if (res.success !== true) {
		...
		editDialog(orderInfo) {
			...
			this.seletedStatusId = orderInfo.state;
