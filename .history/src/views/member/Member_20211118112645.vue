			...
			<el-row :gutter="24" class="search">
				<el-col :span="6">
					<el-input
						...
						clearable
						@clear="_getAllMember"
						@keyup.enter.native="_searchMember"
					>
						<el-button
							slot="append"
							icon="el-icon-search"
...
export default {
	...
	methods: {
		async _getAllMember() {
			let total = await getAllMember();
			this.total = total.data;
			let res = await getMemberLimit(this.queryInfo);
			if (res.success === true) {
		...
		//查找用户
		async _searchMember() {
			let res = await searchMember(this.queryInfo.query);
			if (res.success === true) {
				this.memberList = res.data;
		...
		async _addMember() {
			this.$refs.addFormRef.validate(async (valid) => {
				...
				if (this.addForm.id === "") {
					//添加
					let res = await addMember(this.addForm);
					if (res.success === true) {
						this.message(res.message, "success");
					} else {
						this.message(res.message, "error");
					...
					//修改
					let res = await updateMember(this.addForm);
					if (res.success === true) {
						this.message(res.message, "success");
					} else {
						this.message(res.message, "error");
